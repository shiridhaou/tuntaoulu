        ...
        <div className="flex flex-col gap-3 min-h-0">
          ...
          <div className="rounded-2xl border border-border bg-card p-4 flex flex-col gap-4">
            <div className="flex items-center gap-4">
              <button onClick={() => adjust(-0.05)} className="h-14 w-14 rounded-xl bg-fed-red/15 border border-fed-red/40 text-fed-red hover:bg-fed-red/25 active:scale-95 transition-all flex items-center justify-center" aria-label="decrease 0.05">
              ...
              <div className="flex-1 px-2">
                <Slider
                  ...
                  max={max}
                  step={0.05}
                  onValueChange={(v) => { setEntry(null); setJudgeBScore(myIndex, +v[0].toFixed(2)); }}
              ...
              <button onClick={() => adjust(0.05)} className="h-14 w-14 rounded-xl bg-green-500/15 border border-green-500/40 text-green-400 hover:bg-green-500/25 active:scale-95 transition-all flex items-center justify-center" aria-label="increase 0.05">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[-0.1, -0.05, 0.05, 0.1].map((d) => (
                <button
                  key={d}
                  onClick={() => adjust(d)}
                  className={`h-12 rounded-xl border font-heading font-black text-base active:scale-95 transition-all ${
                    d < 0
            ...
            <div className="grid grid-cols-3 gap-2">
              {["1","2","3","4","5","6","7","8","9",".","0"].map((k) => (
                <button
                  key={k}
                  onClick={() => pressKey(k)}
                  className="h-14 rounded-xl border border-border bg-navy-light text-2xl font-heading font-black text-foreground hover:border-gold/50 hover:text-gold active:scale-95 transition-all"
                >
                  {k}
                </button>
              ))}
